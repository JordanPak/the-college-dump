...
if ($user_ID) {
...

	{
		...
		$message = "<?php _e( 'Your profile updated successfully.', 'agrg' ); ?>";
		...
		update_user_meta( $user_ID, 'flatads_author_avatar_url', $your_image_url );
		...
		if(isset($email)) {
			...
			else { $message = "<div id='error'><?php _e( 'Please enter a valid email id.', 'agrg' ); ?></div>"; }
		...
		if($password) {
			...
			if (strlen($password) < 5 || strlen($password) > 15) {
				...
				$message = "<div id='error'><?php _e( 'Password must be 5 to 15 characters in length.', 'agrg' ); ?></div>";
			...
			elseif(isset($password) && $password != $confirm_password) {
				...
				$message = "<div class='error'><?php _e( 'Password Mismatch.', 'agrg' ); ?></div>";

			} elseif ( isset($password) && !empty($password) ) {

				$update = wp_set_password( $password, $user_ID );

				$message = "<div id='success'><?php _e( 'Your profile updated successfully.', 'agrg' ); ?></div>";
					...
					<form class="form-item" action="" id="primaryPostForm" method="POST" enctype="multipart/form-data">
						...
						?>
							...
							<fieldset class="input-title">
								...
								<label for="edit-title" class="control-label"><?php _e('Avatar:', 'agrg') ?></label>
								...
								<span class="full-width-button"><a href="#" class="delete-author-image"><i class="fa fa-trash-o"></i><?php _e( 'Delete Image', 'agrg' ); ?></a></span>
								...
								<script>
									...
									jQuery(document).on('click','.upload-author-image', function(e) {
										...
										image_custom_uploader.on('select', function() {
										    ...
										    $thisItem.parent().parent().find( ".criteria-image-url" ).attr({
										        value: url
										    });
										    $thisItem.parent().parent().find( ".criteria-image-id" ).attr({
										        value: attachId
										    });
										});
